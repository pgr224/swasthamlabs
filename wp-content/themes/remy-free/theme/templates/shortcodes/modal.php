...
<div class="modal-shortcode">
    <div class="modal-opener" data-toggle="modal" data-target="#modal-<?php echo $seed?>">
        <?php
            if ( $opener == "text" ):
                ?>
        <a title="<?php _e( 'Open Modal', 'yit' )?>" style="font-size: <?php echo ( ! empty( $link_text_size ) ) ? $link_text_size : 17?>px">
            <?php
                if( $link_icon_type == 'theme-icon' ){?>
                    <span class="fa fa-<?php echo $link_icon_theme; ?>"></span>
                <?php
                }
