...
<div id="post-<?php the_ID(); ?>" <?php post_class($featured_image_class); ?>>
    <div class="meta clearfix blog <?php echo $blog_type ?>">
            ...
            <div class="yit_post_content clearfix <?php echo $show_thumbnail == 'thumbnail' && ! $is_quote ? 'col-sm-3 col-sm-pull-9' : '' ?> <?php echo $show_meta_box ? 'show-metabox' : 'hide-metabox' ?> ">
                <?php if ($is_quote) : ?>
                    <style>
                        #post-<?php the_ID(); ?>.post.format-quote .blog.big .yit_post_content {
                            background: url('<?php echo wp_get_attachment_url( get_post_thumbnail_id(get_the_ID()) );?>') top center no-repeat;
                            background-size: cover;
                        }
                    </style>
                    <?php $quote_args = array(
                ...
                <?php else : ?>
                    <?php if ($blog_type == 'big' && $show_date && !is_singular('post')) : ?>
