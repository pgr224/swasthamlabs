...
function instagram_fetch_items($keyword,$camp ){
	...
	//wp_automatic_it_sessionid
	$wp_automatic_it_sessionid = get_option('wp_automatic_it_sessionid','');
	
	if(trim($wp_automatic_it_sessionid) == ''){
		echo '<br><span style="color:red;">Please visit the plugin settings page and insert the required Instagram Session ID</span>';
		return false;
	}else{
		
		//set the session
		curl_setopt($this->ch,CURLOPT_COOKIE,'sessionid='.trim($wp_automatic_it_sessionid));
	...
	if( isset($jsonArr->status) ){
		...
		if(in_array('OPT_IT_POPULAR', $camp_opt) && ! in_array('OPT_IT_USER', $camp_opt)){
			$items = $jsonArr->top_posts->nodes;
